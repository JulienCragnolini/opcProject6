export default class Board {
    ...
    elsePlacement (id, color, name, object) {
        ...
        console.log(this.tabCases[alea].X + "/" + this.tabCases[alea].Y + name);
    ...
    detailsPlacement(nvrVar, nameCase, colorCase, wordCase, object) {
        for (let i = 0; i < nvrVar; i++) {
            if ((this.tabCases[this.randomNumber()].id !== "caseFree") || (this.tabCases[this.randomNumber()].id == "caseObstacle" && "casePlayer" && "caseWeapon")) {
                ...
                this.elsePlacement(nameCase, colorCase, wordCase, object);
            }
        }
    }
}
